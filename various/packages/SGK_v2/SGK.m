...
%     rPerm = 1:size(Dictionary,2);
    ...
    if (displayErrorWithTrueDictionary )
        [ratio(iterNum+1),ErrorBetweenDictionaries(iterNum+1)] = I_findDistanseBetweenDictionaries(normc(param.TrueDictionary),normc(Dictionary));
...
% Dmark = dictionary_marker(Dictionary,Dictionary,CoefMatrix,epsilon);
% output.Dmark = [FixedDictionaryElement,Dmark];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  dictionary_marker
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function Dmark = dictionary_marker(Dmark,Dictionary,CoefMatrix,epsilon)
% D = normc(Dictionary);
% T2 = 0.99;
% K=size(D,2);
% G=D'*D; G = G-diag(diag(G));
% for jj=1:1:K,
%     if max(G(jj,:))>T2 || sum(CoefMatrix(jj,:).^2)<=8*epsilon^2 ,
%         Dmark(:,jj) = 1;
%     end;
% end;
...
% % the better dictionary element is ubtained using least squire solution.
% % This is because we would like to minimize || errors - beta*element ||_F^2.
% % that is, to approximate the matrix 'errors' with the obtained 'element'
% % and a better dictionary 'element'. i.e. 'beta*element'.
%%%%--------Main SGK update step-----
betterDictionaryElement = (errors*CoefVector')./sum(CoefVector.^2);

function [D,Gamma,err,gerr] = ksvd3(params,varargin)
...
% len = sum(params.sam);
% params.temp = zeros(size(params.data,1), len);
% k=1;
% for i = 1:size(params.data,2)
%     if (params.sam(i) == 1)
%         params.temp(:,k) = params.data(:,i);
%         k = k+1;
%     end
% end
% 
...
if (exactsvd)
%   [atom,s,gamma_j] = svds(X(:,data_indices) - D*smallGamma + Dj*gamma_j, 2);

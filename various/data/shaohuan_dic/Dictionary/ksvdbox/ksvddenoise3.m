function [y,D,nz,params] = ksvddenoise3(params,msgdelta)
...
ids = cell(p,1);
if (p==1)
  ids{1} = reggrid(length(traindata)-blocksize+1, trainnum, 'eqdist');
else
  [ids{:}] = reggrid(size(traindata)-blocksize+1, trainnum, 'eqdist');
end
params.data = sampgrid(traindata,blocksize,ids{:});
...
tic;
[D, Gamma,err] = ksvd3(params,verbose,msgdelta);
params.dur = toc;
params.mean = err;
